...
export function BoardEmployeePreview({ emp, onChangePresence }) {
    ...
    const onChangePresenceClick = async (employeeId, isPresence) => {
        document.getElementById(`${employeeId}`).classList.toggle('gray')
        console.log("HERE?");
        setIsLoading(true)
        await onChangePresence(employeeId, !isPresence)
        setIsLoading(false)
    }
    return (
        <article id={`${emp._id}-card`} className="employee-card" onClick={() => onChangePresenceClick(emp._id, emp.isPresence)}>
