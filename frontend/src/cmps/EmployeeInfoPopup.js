...
export const EmployeeInfoPopup = ({ employee, handleClose, notifyCopyNFC, handleClickAway}) => {
    ...
    return (
        <div className="popup-box">
            <div className="box infobox">
                <span className="info-close-icon close-icon" onClick={handleClose}>x</span>
                <div className='employee-profile'>
                    <img className='employee-profile-image' src={employee.image} />
                    <div className='employee-info'>
                        <h1>{employee.firstName + " " + employee.lastName}</h1>
                        <h2>Status: <span className={employee.isPresence ? 'green' : 'red'}>{employee.isPresence ? "Here" : "Out Of Office"}</span></h2>
