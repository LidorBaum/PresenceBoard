...
const snackError500 = { severity: 'error', open: true, message: 'There was an Error with the server, please try again' }
const snackEmployeeSaved = { severity: 'success', open: true, message: 'The employee saved successfully' }
export const EditEmployeePopup = ({employeeToEdit, companyId, company, handleClose, handleClickAway}) => {
    ...
    const [primaryImgUrl, setPrimaryUrl] = useState(employeeToEdit.image || 'https://res.cloudinary.com/echoshare/image/upload/v1638211337/1997805_dje7p6.png')
    ...
    const onAddEmployee = async (e) => {
        ...
        if (primaryImgUrl === 'https://res.cloudinary.com/echoshare/image/upload/v1638211337/1997805_dje7p6.png') {
            setIsLoading(false)
            if (snack.open) {
        ...
        if (employeeToEdit._id) {
            ...
            if (snack.open) {
                setSnack(prevSnack => { return { ...prevSnack, open: false } })
                setTimeout(() => { setSnack(snackEmployeeSaved) }, 100)
            }
            ...
            handleClose('update')
                ...
                if (err.response?.status === 500 || !err.response) {
                    if (snack.open) {
                        setSnack(prevSnack => { return { ...prevSnack, open: false } })
                        setTimeout(() => { setSnack(snackError500) }, 100)
                    }
                ...
                <div className='popup-content'>
                    <form id='edit-form' onSubmit={onAddEmployee}>
                        <div className='form-fields'>
                            <TextField autoComplete='off' required label="First Name" inputProps={{ minLength: 3 }} name='firstName' placeholder='Employee First Name' value={empForm.firstName} onChange={handleChange} />
                            <TextField autoComplete='off' required label="Last Name" inputProps={{ minLength: 3 }} name='lastName' placeholder='Employee Last Name' value={empForm.lastName} onChange={handleChange} />
                            <TextField label="Company" name='company-name' value={company} InputProps={{ readOnly: true }} />
                        </div>
                    ...
                    <Button form='edit-form' type='submit' disabled={isUploading || isLoading}>{employeeToEdit._id ? 'Save' : 'Add'}</Button>
                    {/* <img src={}></img> */}
                </div>
            </div>
        </div>
