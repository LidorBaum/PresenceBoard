...
employeeRouter.get(
    '/presence/:employeeId([A-Fa-f0-9]{24})',
    updateEmployeePresenceNFC
);

employeeRouter.put('/presenceAll', _setAllPresence);

employeeRouter.get(
    '/company/:companyId([A-Fa-f0-9]{24})/:sort',
...
async function updateEmployeePresenceNFC(req, res) {
    try {
        let socket;
        if (env === 'prod') socket = io.connect(baseURL, { secure: true });
        else socket = io.connect(baseURL);
